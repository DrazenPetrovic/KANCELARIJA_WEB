...
// ---------------------------------------------------------------------------
// GET /api/invoices/:invoiceNumber — preuzimanje kompletnog sadržaja već
// fiskalizovanog računa (uključujući sliku računa) po njegovom invoiceNumber-u
// (= br_fiskalnog). Zajednička za gotovinske i žiralne račune. Opcije layout/
// format/slip idu kao query string, isti nazivi kao kod POST /api/invoices —
// vidi ESIR_SLIP_PRESET_58MM / _80MM za receiptSlip* vrijednosti.
// ---------------------------------------------------------------------------

export type EsirOpcijePreuzimanjaRacuna = Pick<
  EsirOpcijeStampe,
  | "receiptLayout"
  | "receiptImageFormat"
  | "receiptSlipWidth"
...
export async function preuzmiFiskalniRacun(
  uredjaj: EsirUredjaj,
  invoiceNumber: string,
  opcije: EsirOpcijePreuzimanjaRacuna = {},
): Promise<EsirInvoiceResponse> {
  const params = new URLSearchParams();
  for (const [kljuc, vrijednost] of Object.entries(opcije)) {
    if (vrijednost !== undefined) params.set(kljuc, String(vrijednost));
  }
  const query = params.toString();
  const res = await esirFetch(
    uredjaj,
    `/api/invoices/${encodeURIComponent(invoiceNumber)}${query ? `?${query}` : ""}`,
    { method: "GET" },
  );
  const rawText = await res.text();
  if (!res.ok) {
    console.error(
      "ESIR GET /api/invoices/:invoiceNumber — sirov odgovor greške:",
      rawText,
    );
    throw new Error(izdvojiEsirGresku(rawText, res.status));
  }
  // Odgovor je omotan — { invoiceRequest, invoiceResponse: {...}, fiscalizedPayment,
  // autoGenerated }, ne ravan EsirInvoiceResponse (za razliku od POST /api/invoices) —
  // vidi izdvojiInvoiceResponse.
  let parsed: unknown = null;
  try {
    parsed = JSON.parse(rawText);
  } catch {
  ...
  const { invoiceResponse } = izdvojiInvoiceResponse(parsed);
  if (!invoiceResponse) {
    throw new Error("ESIR GET /api/invoices/:invoiceNumber — neočekivan oblik odgovora.");
  }
