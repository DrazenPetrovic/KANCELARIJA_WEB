...
import { KufPregledTemplate } from "../print/templates/KufPregledTemplate";

const API_URL = import.meta.env.VITE_API_URL || "http://localhost:3002";
const PRIMARY = "#785E9E";
const ACCENT = "#8FC74A";

// Red vraćen sa erp.kuf_pregled() (preko preglediRazni.service getKuf()).
interface KufRed {
  sifra: number;
  ...
  sif_drzave: number;
  datum_ulaza_dokumenta: string;
  grad_partnera: string;
  drzava_partnera: string;
  entitet_partnera: string;
  JIB: string;
...
const TIP_DOKUMENTA_NAZIV: Record<string, string> = {
...
const opisTipaDokumenta = (v: string | null | undefined): string | null =>
  v ? (TIP_DOKUMENTA_NAZIV[v] ?? null) : null;
...
function StatTile({
  ...
  // vrijednost izračunata (zbir dvije kolone), a ne sirov zbir kolone.
  izracunato?: boolean;
}) {
  return (
    <div
...
// poslije, isti pristup kao u Kif.tsx.
...
export function Kuf() {
  ...
  const [redovi, setRedovi] = useState<KufRed[]>([]);
  ...
  const ucitaj = () => {
    setLoading(true);
    setGreska(null);
    fetch(`${API_URL}/api/pregledi-razni/kuf`, { credentials: "include" })
      .then((res) => {
        if (!res.ok) throw new Error("Greška pri učitavanju KUF-a");
        return res.json();
      })
  ...
  // kuf_pregled ne prima parametre (vraća sve stavke) — period se filtrira ovdje.
  useEffect(ucitaj, []);
  ...
  const redoviFiltrirano = useMemo(() => {
    ...
    return redovi
      .filter((r) => {
        const d = new Date(r.datum_ulaza_dokumenta);
      ...
      .sort(
        (a, b) =>
          new Date(b.datum_ulaza_dokumenta).getTime() -
          new Date(a.datum_ulaza_dokumenta).getTime(),
      );
  ...
  const ukupnoVrednostBezPdv = useMemo(
    () =>
      redoviFiltrirano.reduce((s, r) => s + Number(r.vrednost_bez_pdv || 0), 0),
    [redoviFiltrirano],
  );
  const ukupnoIskazaniPdv = useMemo(
    () => redoviFiltrirano.reduce((s, r) => s + Number(r.iskazani_pdv || 0), 0),
    [redoviFiltrirano],
  );
  const ukupnoUlazniPdv = useMemo(
    () => redoviFiltrirano.reduce((s, r) => s + Number(r.ulazni_pdv || 0), 0),
    [redoviFiltrirano],
  );
  const ukupnoUlazniPdvUvoz = useMemo(
    () =>
      redoviFiltrirano.reduce((s, r) => s + Number(r.ulazni_pdv_uvoz || 0), 0),
    [redoviFiltrirano],
  );
  // kuf_pregled ne vraća gotov "ukupno" — izvodi se na frontendu kao osnovica +
  // iskazani PDV, isti obrazac kao ukupno=osnovica+pdv u Kif.tsx.
  const ukupnoUkupno = useMemo(
    () => ukupnoVrednostBezPdv + ukupnoIskazaniPdv,
    [ukupnoVrednostBezPdv, ukupnoIskazaniPdv],
  );
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-400 dark:text-[#5f5878]">
            Knjiga ulaznih faktura za izabrani period
          </p>
        </div>
        <button
          type="button"
          onClick={() =>
            openPrint({
              title: `KUF ${formatDatumDMY(primenjeniOd) ?? ""} - ${formatDatumDMY(primenjeniDo) ?? ""}`,
      ...
      {!loading && !greska && (
        <div className="flex flex-wrap gap-3">
          ...
          <StatTile
            ...
            vrijednost={formatIznos(ukupnoVrednostBezPdv)}
            naziv="Vrijednost bez PDV"
            boja={PRIMARY}
          />
          <StatTile
            icon={<Receipt size={16} />}
            vrijednost={formatIznos(ukupnoIskazaniPdv)}
            naziv="Iskazani PDV"
            boja={PRIMARY}
          />
          <StatTile
            icon={<Tags size={16} />}
            vrijednost={formatIznos(ukupnoUlazniPdv)}
            naziv="Ulazni PDV"
            boja={PRIMARY}
          />
          <StatTile
            icon={<Tags size={16} />}
            vrijednost={formatIznos(ukupnoUlazniPdvUvoz)}
            naziv="Ulazni PDV (uvoz)"
            boja={PRIMARY}
          />
        </div>
      )}
            ...
            <table className="w-full text-sm">
                  ...
                  >
                    ...
                    <TD>
                      <span
                        title={
                          r.datum_dokumenta
                            ? `Datum dokumenta: ${formatDatumDMY(r.datum_dokumenta)}`
                            : undefined
                        }
                      >
                        {formatDatumVrijemeDMY(r.datum_ulaza_dokumenta) ?? "—"}
                      </span>
                    </TD>
                    <TD>
                      ...
                      <div
                        ...
                        style={{ color: ACCENT }}
                        title={opisTipaDokumenta(r.tip_dokumenta) ?? undefined}
                      >
                        {opisTipaDokumenta(r.tip_dokumenta) ?? "—"}
                      </div>
                    </TD>
                    <TD>
                    ...
                    <TD>{r.entitet_partnera || "—"}</TD>
                    <TD>{r.opis || "—"}</TD>
                    <TD right>
                      <div>{formatIznos(r.vrednost_bez_pdv)}</div>
                      <div className="text-[10px] text-gray-400 dark:text-[#5f5878]">
                        {formatIznos(r.iskazani_pdv)}
                      </div>
                    </TD>
                    <TD right bold>
                      {formatIznos(
                        Number(r.vrednost_bez_pdv || 0) +
                          Number(r.iskazani_pdv || 0),
                      )}
                    </TD>
                  </tr>
                ))}
              ...
              <tfoot>
                <tr className="border-t-2 border-gray-200 dark:border-[#3a3158] bg-[#faf9fc] dark:bg-[#1e1a2d]">
                  ...
                  <TD>{""}</TD>
                  <TD right>
                    <div>{formatIznos(ukupnoVrednostBezPdv)}</div>
                    <div className="text-[10px] text-gray-400 dark:text-[#5f5878]">
                      {formatIznos(ukupnoIskazaniPdv)}
                    </div>
                  </TD>
                  <TD right bold>
