...
import {
  DostavaTereniProizvodiTemplate,
  type DostavaTereniProizvodiRow,
} from "../print/templates/DostavaTereniProizvodiTemplate";
...
export function NarudzbePregled() {
  ...
  const handleStampajTeren = async () => {
    if (!selectedDay) {
      alert("Odaberi dan prije štampe.");
      return;
    }
    try {
      const res = await fetch(
        `${API_URL}/api/narudzbe/dostava-tereni-proizvodi?sifraTerena=${selectedDay}`,
        {
          method: "GET",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
      ...
      const json = await res.json();
      if (!res.ok || !json?.success) {
        alert(json?.error || "Greška pri učitavanju podataka za štampu");
        return;
      }
      if (!json.data || json.data.length === 0) {
        alert(`Nema podataka za štampu (sifra_terena=${selectedDay}).`);
      }
      const dayLabel = uniqueDays.find(
        (d) => d.sifraTerenaDostava === selectedDay,
      )?.date;
      openPrint({
        title: `Dostava — teren${dayLabel ? ` za ${dayLabel}` : ""}`,
        component: (
          <DostavaTereniProizvodiTemplate
            rows={json.data as DostavaTereniProizvodiRow[]}
            terenLabel={dayLabel}
          />
        ),
      });
    } catch {
      alert("Greška pri učitavanju podataka za štampu");
    }
  };
