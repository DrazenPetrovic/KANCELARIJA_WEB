...
import {
  ZavrseneLokalnoTemplate,
  type PartnerSection,
} from "../print/templates/ZavrseneLokalnoTemplate";
...
export function NarudzbeZavrseneLokalno() {
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        if (!json.success) throw new Error(json.error || "Greška pri dohvatu narudžbi");
        if (!cancelled) setOrders(Array.isArray(json.data) ? json.data : []);
      } catch (loadError) {
        if (!cancelled)
          setError(loadError instanceof Error ? loadError.message : "Nema veze sa serverom");
      } finally {
  ...
  useEffect(() => {
    ...
    const loadItems = async () => {
      ...
      setItemsError(null);
      try {
        const response = await fetch(`${API_URL}/api/trade-orders/finished/items`, {
          credentials: "include",
        });
        const json = await response.json();
        if (!json.success) throw new Error(json.error || "Greška pri dohvatu stavki");
        if (cancelled) return;

        const items = Array.isArray(json.data) ? json.data : [];
        const next: Record<number, OrderItem[]> = {};
        ...
        if (!cancelled)
          setItemsError(loadError instanceof Error ? loadError.message : "Greška pri dohvatu stavki");
      } finally {
        if (!cancelled) setItemsIndexing(false);
      }
    };
  ...
  const uniquePartners = useMemo(() => {
    ...
    for (const order of orders) {
      if (!seen.has(order.partner_id)) {
        seen.add(order.partner_id);
        result.push({ id: order.partner_id, name: order.partner_name });
      }
    }
    return result.sort((a, b) => a.name.localeCompare(b.name, "bs"));
  }, [orders]);

  const togglePartner = (partnerId: number, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handlePrint = () => {
    ...
    const partners: PartnerSection[] = uniquePartners
      .filter((p) => selectedPartnerIds.has(p.id))
      .map((p) => ({
        id: p.id,
        name: p.name,
        orders: orders
          .filter((o) => o.partner_id === p.id)
          .sort((a, b) => {
    ...
    const title =
      partners.length === 1
        ? `Završene lok. narudžbe — ${partners[0].name}`
        : `Završene lok. narudžbe — ${partners.length} partnera`;
    ...
    const matchesProduct =
      ...
      orderItems.some(
        (item) =>
          ...
          (item.product_group || "").toLowerCase().includes(normalizedProductQuery),
      );
  ...
  const sortedOrders = [...filteredOrders].sort((left, right) => {
    const lt = left.updated_at
      ? new Date(left.updated_at).getTime()
      : new Date(left.created_at).getTime();
    const rt = right.updated_at
  ...
  const selectedOrderCount = orders.filter((o) => selectedPartnerIds.has(o.partner_id)).length;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 dark:text-[#5f5878]">
              Označite partnere i kliknite Štampaj za ispis narudžbi.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white px-4 py-3 dark:border-[#3a3158] dark:bg-[#1e1a2d]">
          ...
          <div className="mt-1 flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-[#c5bfd8]">
            ...
            {itemsIndexing ? "Učitavanje stavki za pretragu" : "Spreman za filtriranje"}
          </div>
          {itemsError && (
            <div className="mt-2 text-xs text-red-600 dark:text-red-400">{itemsError}</div>
          )}
        </div>
      </div>

      {/* Print traka */}
      <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3 dark:border-[#3a3158] dark:bg-[#1e1a2d]">
        <Printer size={15} style={{ color: PRIMARY }} className="flex-shrink-0" />
        <span className="flex-1 text-sm text-gray-500 dark:text-[#7d7498]">
          {selectedCount === 0
            ? "Označite partnere u listi za štampu"
            : `Izabrano: ${selectedCount} ${selectedCount === 1 ? "partner" : "partnera"} · ${selectedOrderCount} narudžbi`}
        </span>
        {selectedCount > 0 && (
          <button
            onClick={() => setSelectedPartnerIds(new Set())}
            className="text-xs text-gray-400 hover:text-gray-600 dark:text-[#5f5878] dark:hover:text-[#c5bfd8] transition-colors"
          >
            Obriši izbor
          </button>
        )}
        <button
          onClick={handlePrint}
          disabled={selectedCount === 0 || itemsIndexing}
          className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold text-white transition-all hover:brightness-110 disabled:opacity-40 disabled:cursor-not-allowed"
          style={{ background: PRIMARY }}
        >
          <Printer size={14} />
          {selectedCount > 1 ? `Štampaj (${selectedCount})` : "Štampaj narudžbe"}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-2 py-16 text-gray-400 dark:text-[#5f5878]">
          ...
          <span className="text-sm">
            {activeFilters ? "Nema rezultata za zadatu pretragu" : "Nema završenih narudžbi"}
          </span>
        </div>
      ) : (
        <div className="space-y-3">
          {sortedOrders.map((order) => {
            ...
            const isSelected = selectedPartnerIds.has(order.partner_id);
            const orderItems = itemsByOrder[order.id] ?? [];
            ...
            return (
              <div
                key={order.id}
                className={`flex overflow-hidden rounded-2xl border shadow-sm transition-colors ${
                  isSelected
                    ? "border-[#785E9E] dark:border-[#785E9E]"
                    : "border-gray-200 dark:border-[#3a3158]"
                } bg-white dark:bg-[#1e1a2d]`}
              >
                {/* Checkbox kolona */}
                <div
                  className={`flex items-center justify-center px-3 flex-shrink-0 cursor-pointer transition-colors ${
                    isSelected
                      ? "bg-[#f2edf8] dark:bg-[#2d2648]"
                      : "bg-gray-50 dark:bg-[#1a1628] hover:bg-[#f8f6fc] dark:hover:bg-[#2d2648]"
                  }`}
                  onClick={(e) => togglePartner(order.partner_id, e)}
                >
                  <input
                    type="checkbox"
                    checked={isSelected}
                    ...
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500 dark:text-[#7d7498]">
                          ...
                          <span>Kreirano: {formatDate(order.created_at)}</span>
                          ...
                          {order.updated_at && (
                            <span className="text-gray-400 dark:text-[#5f5878]">
                              Ažurirano: {formatDate(order.updated_at)}
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3 lg:justify-end">
                        <div className="grid grid-cols-2 gap-x-5 gap-y-1 text-xs text-gray-500 dark:text-[#7d7498] sm:grid-cols-4">
                          ...
                          <span>Količina: {formatQty(totalQuantity)}</span>
                          <span>
                          ...
                          <table className="w-full min-w-[780px] text-xs">
                            <thead>
                              <tr className="border-b border-gray-200 dark:border-[#2d2648]">
                                <th className="px-4 py-2 text-left font-bold uppercase tracking-wider text-gray-400 dark:text-[#5f5878]">#</th>
                                <th className="px-2 py-2 text-left font-bold uppercase tracking-wider text-gray-400 dark:text-[#5f5878]">Proizvod</th>
                                <th className="px-2 py-2 text-left font-bold uppercase tracking-wider text-gray-400 dark:text-[#5f5878]">Grupa</th>
                                <th className="px-2 py-2 text-right font-bold uppercase tracking-wider text-gray-400 dark:text-[#5f5878]">Kol.</th>
                                <th className="px-2 py-2 text-left font-bold uppercase tracking-wider text-gray-400 dark:text-[#5f5878]">JM</th>
                                <th className="px-2 py-2 text-right font-bold uppercase tracking-wider text-gray-400 dark:text-[#5f5878]">VPC</th>
                                <th className="px-4 py-2 text-right font-bold uppercase tracking-wider text-gray-400 dark:text-[#5f5878]">MPC</th>
                              </tr>
                            </thead>
                            <tbody>
                              {orderItems.map((item) => (
